...
template <typename T>
static T* UniformDistribution(int rank, size_t* dims, double lower,
                              double upper) {
  ...
  std::default_random_engine generator(
      std::chrono::duration<double>(now - beginning).count() * 10000);
...
int main(int argc, char** argv) {
  ...
  if (args.io_size_ > 0) {
    double total_number_of_ints =
        args.io_size_ * MB / (H5Tget_size(type_) * 1.0);
