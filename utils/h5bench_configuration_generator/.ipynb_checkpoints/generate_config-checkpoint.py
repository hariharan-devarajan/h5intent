...
def parse_args():
    ...
    parser.add_argument("-a", "--scheduler", default="lsf", type=str, help="Scheduler to configure lsf or slurm")
...
def main():
    ...
    if args.scheduler == "lsf":
        ...
        if args.profiler:
            env = f"--export=LD_PRELOAD={args.profiler}"
        mpi = {
            "command": "srun",
            "configuration": f"--ntasks={args.processes_per_node * args.nodes} --ntasks-per-node={args.processes_per_node} {env}"
        }
    #args.vol = "none"
    print(args.vol)
    vol = {}
